...
<head>
    ...
    <title>Optical CRM - @yield('page-title', __('Dashboard'))</title>
...
<body class="font-sans text-slate-900 bg-slate-50 antialiased">
    ...
    <div x-data="{ mobileMenuOpen: false }" class="flex h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-white to-indigo-50/40 dark-bg no-bounce">
        ...
        <!-- SIDEBAR -->
        <!-- ========================================== -->
                <aside class="fixed inset-y-0 start-0 z-40 w-72 transform bg-gradient-to-b from-indigo-950 via-slate-950 to-slate-950 text-slate-100 shadow-2xl transition duration-300 md:translate-x-0"
               :class="mobileMenuOpen ? 'translate-x-0' : '-translate-x-full rtl:translate-x-full'">
...
<div class="flex items-center justify-between px-6 py-6 border-b border-white/10 flex-shrink-0">
    ...
    <line x1="9" y1="21" x2="9" y2="9"/>
        ...
        <div>
            ...
            <p class="text-xs font-medium text-indigo-300/80">{{ __('Internal management') }}</p>
        </div>
    </div>
    <button type="button" class="inline-flex items-center justify-center rounded-lg border border-white/10 bg-white/5 p-2 text-slate-300 hover:bg-white/10 md:hidden"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-5 w-5">
            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 011.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                ...
                <nav class="flex-1 overflow-y-auto px-4 py-6">
                    <p class="px-4 pb-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-indigo-300/70">{{ __('Main Menu') }}</p>
                    ...
                    <!-- Administration -->
                    <p class="px-4 pb-2 pt-8 text-[11px] font-semibold uppercase tracking-[0.28em] text-indigo-300/50">{{ __('Administration') }}</p>
                    <div class="space-y-1.5">
                ...
                <div class="border-t border-white/10 px-6 py-5 flex-shrink-0">
                    <div class="rounded-2xl bg-gradient-to-br from-indigo-500/20 to-fuchsia-500/10 border border-white/10 p-4">
                        <p class="text-xs uppercase tracking-[0.2em] text-indigo-300">{{ __('Need help?') }}</p>
                        <p class="mt-2 text-sm leading-6 text-slate-300">{{ __('Contact the IT team for any internal support request.') }}</p>
                        <button @click="$store.theme.toggle()" 
                                ...
                                x-text="$store.theme.dark ? '{{ __('Light mode') }}' : '{{ __('Dark mode') }}'">
                        </button>
                    </div>
                </div>
            </div>
        ...
        <!-- ========================================== -->
        <div class="flex flex-1 flex-col overflow-hidden md:ps-72">
            ...
            <header class="flex-shrink-0 z-20 border-b border-slate-200/70 bg-white/90 backdrop-blur-md">
                <div class="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="flex min-w-0 items-center gap-4">
                        <button type="button" class="inline-flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-700 md:hidden" @click="mobileMenuOpen = true">
                        ...
                        <div class="min-w-0">
                            <p class="text-xs uppercase tracking-[0.24em] text-indigo-500 font-semibold">{{ __('Internal interface') }}</p>
                            <h1 class="truncate text-lg font-semibold tracking-tight text-slate-900">@yield('page-title' , __('Dashboard'))</h1>
                        </div>
                    </div>
                    ...
                    <div class="flex min-w-0 flex-1 items-center justify-end gap-2 sm:gap-3">
                        ...
                        <!-- Global Search -->
                        <div class="relative hidden w-full max-w-md md:block" x-data="{
                            query: '',
                            results: [],
                            loading: false,
                            searchOpen: false,
                            ...
                            <label class="flex h-9 w-full cursor-text items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 ps-3 pe-3 transition focus-within:border-indigo-500 focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo-500/20">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 flex-shrink-0 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <circle cx="11" cy="11" r="8"></circle>
                                    <path d="m21 21-4.3-4.3"></path>
                                </svg>
                                <input type="text" x-model="query" @input="doSearch()" @focus="searchOpen = true"
                                       @keydown.escape.prevent="searchOpen = false"
                                       placeholder="{{ __('Search...') }}"
                                       class="h-full w-full min-w-0 bg-transparent text-sm text-slate-700 placeholder-slate-400 focus:outline-none">
                            </label>

                            <!-- Search Results Dropdown -->
                            <div x-show="searchOpen && query.trim().length >= 2" x-cloak @click.away="searchOpen = false"
                                 x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
                                 x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
                                 class="absolute start-0 mt-2 w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl ring-1 ring-black ring-opacity-5 z-50">

                                <div x-show="loading" class="flex items-center gap-2 px-4 py-3 text-sm text-slate-500">
                                ...
                                <template x-for="group in grouped" :key="group.name">
                                    <div>
                                        <p class="px-4 pt-3 pb-1 text-[11px] font-semibold uppercase tracking-wider text-slate-400" x-text="group.name"></p>
                                        <template x-for="item in group.items" :key="item.url">
                                            <a :href="item.url" class="flex items-start justify-between gap-3 px-4 py-2.5 text-sm transition hover:bg-indigo-50/50">
                                                <span class="min-w-0">
                                                    <span class="block truncate font-medium text-slate-800" x-text="item.title"></span>
                                                    <span class="block truncate text-xs text-slate-500" x-text="item.subtitle"></span>
                                                </span>
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 flex-shrink-0 self-center text-slate-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"></path></svg>
                                            </a>
                                        </template>
                                    </div>
                                </template>

                                <p x-show="!loading && results.length === 0" class="px-4 py-4 text-sm text-slate-500">{{ __('No results found.') }}</p>
                            </div>
                        </div>

                        <!-- Language Switcher -->
                        <div class="relative flex-shrink-0" x-data="{ langOpen: false }">
                            <button type="button" @click="langOpen = !langOpen" :aria-expanded="langOpen.toString()"
                                    class="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 hover:text-indigo-600">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
                                    <circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>
                                </svg>
                                <span class="uppercase tracking-wide text-xs">{{ app()->getLocale() }}</span>
                            </button>

                            <div x-show="langOpen" @click.away="langOpen = false" x-cloak x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95"
                                 class="absolute end-0 mt-2 w-44 origin-top-right rounded-xl border border-slate-200 bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                                ...
                                @foreach(['fr' => __('French'), 'en' => __('English'), 'ar' => __('Arabic')] as $code => $label)
                                    <a href="{{ route('language.switch', $code) }}"
                                       class="flex items-center justify-between gap-2 px-4 py-2 text-sm transition {{ app()->getLocale() === $code ? 'bg-indigo-50 font-semibold text-indigo-600' : 'text-slate-700 hover:bg-slate-50 hover:text-indigo-600' }}">
                                        <span>{{ $label }}</span>
                                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ $code }}</span>
                                    </a>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="relative flex-shrink-0" x-data="{ 
                            ...
                            <button @click="notifOpen = !notifOpen" class="relative inline-flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-indigo-600">
                                ...
                                <span x-show="unreadCount > 0" x-text="unreadCount" class="absolute -top-1 -end-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white ring-2 ring-white" x-transition></span>
                            </button>
                            ...
                            <div x-show="notifOpen" @click.away="notifOpen = false" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
                                 class="absolute end-0 mt-2 w-80 origin-top-right rounded-xl border border-slate-200 bg-white py-2 shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none z-50 max-h-96 overflow-y-auto">
                                
                                <div class="flex items-center justify-between px-4 py-2 border-b border-slate-100">
                                    <h3 class="text-sm font-bold text-slate-900">{{ __('Notifications') }}</h3>
                                    <button @click="markAllRead()" x-show="unreadCount > 0" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800">{{ __('Mark all as read') }}</button>
                                </div>
                        ...
                        <div class="relative flex-shrink-0" x-data="{ open: false }">
                            <button type="button" class="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 bg-white ps-1.5 pe-2.5 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50" @click="open = !open">
                                ...
                                <span class="hidden max-w-28 truncate text-sm font-medium text-slate-700 md:block">{{ optional(Auth::user())->name ?? __('User') }}</span>
                            ...
                            <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95"
                                 class="absolute end-0 mt-2 w-56 origin-top-right rounded-xl border border-slate-200 bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
        ...
        <div class="relative min-h-screen flex items-start justify-center p-4 pt-12">
            <div x-show="$store.detailModal.open"
                ...
                <div x-show="$store.detailModal.loading" class="flex items-center justify-center p-12">
                    ...
                    <span class="ms-3 text-sm text-slate-600">{{ __('Loading...') }}</span>
                </div>
            </div>
        </div>
    </div>
