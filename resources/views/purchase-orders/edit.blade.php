...
<div class="mx-auto max-w-5xl space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-2xl font-bold text-slate-900">Modifier le Bon de Commande #{{ $purchaseOrder->order_number }}</h2>
    </div>

    <form method="POST" action="{{ route('purchase-orders.update', $purchaseOrder) }}" class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm space-y-8">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <div>
                ...
                <input type="date" name="order_date" value="{{ old('order_date', $purchaseOrder->order_date->format('Y-m-d')) }}" required class="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700">Date de réception prévue</label>
                <input type="date" name="expected_date" value="{{ old('expected_date', $purchaseOrder->expected_date?->format('Y-m-d')) }}" class="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">
            </div>
        </div>
                ...
                @forelse($purchaseOrder->items as $index => $item)
                    <div class="grid grid-cols-12 gap-3 items-end item-row">
                        <div class="col-span-6">
                            ...
                            <select name="products[{{ $index }}][product_id]" required class="w-full rounded-xl border border-slate-300 px-4 py-3 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">
                                <option value="">Choisir un produit...</option>
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ $item->product_id == $product->id ? 'selected' : '' }}>{{ $product->name }} (Stock: {{ $product->quantity }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-span-2">
                            <label class="block text-xs font-medium text-slate-500 mb-1">Quantité</label>
                            <input type="number" name="products[{{ $index }}][quantity]" min="1" value="{{ $item->quantity }}" required class="w-full rounded-xl border border-slate-300 px-4 py-3 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">
                        </div>
                        <div class="col-span-3">
                            <label class="block text-xs font-medium text-slate-500 mb-1">Coût unitaire (DH)</label>
                            <input type="number" name="products[{{ $index }}][unit_cost]" step="0.01" min="0" value="{{ $item->unit_cost }}" required class="w-full rounded-xl border border-slate-300 px-4 py-3 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">
                        </div>
        ...
        <div>
            ...
            <textarea name="notes" rows="3" class="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">{{ old('notes', $purchaseOrder->notes) }}</textarea>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button type="submit" class="rounded-xl bg-indigo-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/30 hover:bg-indigo-700">Mettre à jour</button>
        </div>
    </form>
</div>

<script>
    let itemCount = {{ max(count($purchaseOrder->items), 1) }};
