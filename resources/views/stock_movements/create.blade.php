...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <p class="text-sm font-semibold uppercase tracking-[0.3em] text-brand-600">Nouveau mouvement</p>
            <h2 class="text-3xl font-semibold text-slate-900">Ajouter un mouvement de stock</h2>
            <p class="mt-2 text-sm text-slate-500">Entrer un mouvement d'entrée ou de sortie.</p>
        </div>
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('stock-movements.store') }}" class="space-y-6">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-brand-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-brand-700">Créer</button>
            </div>
        </form>
    </div>
</div>
